 ...
 */
	...
	TfsColorizer = {	
		...
		initialize : function() {
			...
			try {
				...
				self.port.on( "loadSettingsResponse", TfsColorizer.onLoadSettingsResponseReceived );
				// Load settings
		...
		onLoadSettingsResponseReceived : function ( settings ) {
			//console.log( "TfsColorizer.onLoadSettingsResponseReceived" );
			try {
	...
	self.port.on( "initializeColorizer", function () {
		//console.log( "self.port.on.initializeColorizer" );
		$( document ).find( "h1" ).text( "initialize" );
	});
